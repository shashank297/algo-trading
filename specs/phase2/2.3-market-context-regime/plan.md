...
1. **Trend Features & Scoring**:
   ...
   - Versioned weights: hard-required $R_{20}$ 0.20, $R_{60}$ 0.20, DMA50 ratio 0.15, DMA200 ratio 0.20; optional $R_{120}$ 0.10 and each slope 0.075. Hard features must exist and available weighted evidence must be at least 0.75.
...
2. **Volatility Features & Scoring**:
   ...
   - Hard-required evidence is $\sigma_{20}$ (0.30), $\sigma_{60}$ (0.25), and normalized ATR (0.25); optional volatility percentile and VIX are 0.10 each. Hard features must exist and available weighted evidence must be at least 0.75.

3. **Breadth Features & Scoring**:
   - Evaluated exclusively over PIT universe active at `decision_time`.
   - Required 20/50/200-DMA and advance/decline coverage are measured independently; their minimum must be at least 80% of the full PIT universe.
...
5. **Liquidity Features & Scoring**:
   - $\text{Turnover\_Ratio} = \text{PIT traded value}_{5} / \text{PIT traded value}_{60}$; percentile evidence uses the trailing exact 252-session aggregate and is optional before then.
...
7. **Deterministic Raw Regime Classification Tree**:
   - If critical evidence missing (benchmark history $< 220$ days, breadth coverage $< 80\%$, or missing critical trend/breadth): $\rightarrow \text{INSUFFICIENT\_CONTEXT}$
   - Else if $\text{DD} \le -0.10$ and $(\text{trend\_score} > 0.0 \text{ or } \text{slope}_{50} > 0.0)$ and $\text{breadth\_score} \ge 0.0$ and $\text{downside\_freq} \le 0.10$ and $\text{vol\_shock} \le 0.25$: $\rightarrow \text{RECOVERY}$
   - Else if $\text{trend\_score} \le -0.20$ and $(\text{breadth\_score} \le -0.10 \text{ or } \text{stress\_score} \ge 0.45)$: $\rightarrow \text{BEAR\_HIGH\_VOL}$
   - Else if $\text{trend\_score} \ge +0.25$ and $\text{breadth\_score} \ge +0.15$ and $\text{volatility\_score} \le +0.15$ and $\text{stress\_score} \le 0.40$: $\rightarrow \text{BULL\_LOW\_VOL}$
   - Else if $\text{trend\_score} \ge +0.25$ and $\text{breadth\_score} \ge +0.05$ and $\text{volatility\_score} > +0.15$: $\rightarrow \text{BULL\_HIGH\_VOL}$
   - Else if $\text{volatility\_score} > +0.15$ or $\text{stress\_score} \ge 0.45$: $\rightarrow \text{SIDEWAYS\_HIGH\_VOL}$
