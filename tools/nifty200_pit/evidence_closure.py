...
def _write_monthly_governance(root: Path, monthly_rows: list[dict[str, str]]) -> None:
    period_counts = Counter(row.get("status", "UNCLASSIFIED") for row in monthly_rows)
    evidence_counts = Counter(row.get("source_issue", "UNCLASSIFIED") for row in monthly_rows)
    gap_periods = [row for row in monthly_rows if row.get("status") != "PASS"]
    lines = [
        ...
        f"Non-passing monthly checkpoint periods: {len(gap_periods)}.",
        "The period denominator is one row per campaign month; it is distinct from",
        f"the underlying evidence-gap record denominator ({len(monthly_rows)} records).", "",
        "## Monthly checkpoint periods", "",
        "| Period status | Monthly periods | Required evidence/action |", "| --- | ---: | --- |",
        f"| PASS | {period_counts.get('PASS', 0)} | No action for this gate |",
        f"| A: no snapshot evidence | {period_counts.get('A_NO_SNAPSHOT_EVIDENCE', 0)} | Retrieve an official checkpoint or retain an evidence gap |",
        f"| E1: corrupt/incomplete download | {period_counts.get('E_HTML_RESPONSE_NOT_ARCHIVE', 0)} | Retrieve valid archive bytes or a verified Wayback copy |",
        f"| E2: source present but zero NIFTY-200 rows | {period_counts.get('E_SOURCE_PRESENT_ZERO_ROWS', 0)} | Obtain an official NIFTY-200 member checkpoint; do not substitute another index |",
        "",
        ...
        f"| A: no snapshot evidence | {evidence_counts.get('A_NO_SNAPSHOT_EVIDENCE', 0)} | Retrieve an official checkpoint or retain an evidence gap |",
        f"| B: candidate/zero-row extraction | {evidence_counts.get('B_CANDIDATE_MEMBER_ZERO_ROWS', 0)} | Inspect archive member/table and parser output |",
        f"| C: wrong table/parser | {evidence_counts.get('C_WRONG_TABLE_OR_PARSER', 0)} | Add parser coverage only after source inspection |",
        f"| D: documented non-200 methodology | {evidence_counts.get('D_DOCUMENTED_NON_200', 0)} | Obtain the official exception/methodology document |",
        f"| E1: corrupt/incomplete download | {evidence_counts.get('E_HTML_RESPONSE_NOT_ARCHIVE', 0)} | Retrieve valid archive bytes or a verified Wayback copy |",
        f"| E2: official archive has no NIFTY-200 member file | {evidence_counts.get('A_ARCHIVE_WITHOUT_NIFTY200_MEMBER', 0)} | Obtain an official NIFTY-200 member checkpoint; do not treat other index files as a substitute |",
...
def generate(root: Path) -> dict[str, Any]:
    ...
    current = {
        ...
        "certified_historical_identity_rows": _metric(validation, "certified_historical_identity_rows"),
        "current_snapshot_only_identity_rows": _metric(validation, "current_snapshot_only_identity_rows"),
        "explicit_historical_interval_identity_rows": _metric(validation, "explicit_historical_interval_identity_rows"),
        "manual_review_identity_rows": _metric(validation, "manual_review_identity_rows"),
        "historical_event_identity_rows": _metric(validation, "historical_event_identity_rows"),
        "certified_historical_event_identity_rows": _metric(validation, "certified_historical_event_identity_rows"),
        "unresolved_historical_event_identity_cases": _metric(validation, "unresolved_historical_event_identity_cases"),
        "manual_review_historical_event_identity_rows": _metric(validation, "manual_review_historical_event_identity_rows"),
        "certified_identity_resolution_pct_among_historical_event_rows": _metric(validation, "certified_identity_resolution_pct_among_historical_event_rows"),
        "isin_resolution_pct_among_historical_event_rows": _metric(validation, "isin_resolution_pct_among_historical_event_rows"),
    ...
    labels = (
        ...
        ("Certified historical identity rows", "certified_historical_identity_rows"),
        ("Current-snapshot-only identity rows", "current_snapshot_only_identity_rows"),
        ("Explicit historical-interval identity rows", "explicit_historical_interval_identity_rows"),
        ("Manual-review identity rows", "manual_review_identity_rows"),
        ("Historical event identity rows (denominator)", "historical_event_identity_rows"),
        ("Certified historical event identity rows", "certified_historical_event_identity_rows"),
        ("Certified historical event identity resolution %", "certified_identity_resolution_pct_among_historical_event_rows"),
        ("ISIN resolution % among historical event rows", "isin_resolution_pct_among_historical_event_rows"),
        ("Unresolved historical event identity cases", "unresolved_historical_event_identity_cases"),
        ("Manual-review historical event identity rows", "manual_review_historical_event_identity_rows"),
